...
async function main() {
  try {
    ...
    console.log('Recovery Note rule table v6 validation passed');
    ...
    console.log(`pending_removal: ${report.pendingRemoval.join(', ') || 'none'}`);
    console.log(`implementation drift: ${report.implementationDrift.join(', ') || 'none'}`);
    console.log(`excluded ingredients: ${report.excludedIngredients.join(', ') || 'none'}`);
